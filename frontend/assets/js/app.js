document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLayout(layout) {
        if (layout.timer) {
            Object.assign(timerContainer.style, layout.timer.position);
            const size = layout.timer.size || { width: '24rem', height: '24rem' };
            timerContainer.querySelector('.relative').style.width = size.width;
            timerContainer.querySelector('.relative').style.height = size.height;
        }
        if (layout.teams) {
            Object.assign(teamsContainer.style, layout.teams.position);
            if(layout.teams.size) Object.assign(teamsContainer.style, layout.teams.size);
        }
        if (layout.note) {
            Object.assign(noteContainer.style, layout.note.position);
            if(layout.note.size) Object.assign(noteContainer.style, layout.note.size);
        }
    }
